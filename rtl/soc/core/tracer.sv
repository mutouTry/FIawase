...
// FIawase: simulation-only. `string`, $fopen and $fwrite are not
// synthesisable, and tinyriscv_soc_top sets TRACE_ENABLE = 0, so this module
// is never elaborated in synthesis -- but `analyze -autoread` still parses
// every file it finds and reports 21 VER-700 errors here. They are harmless
// and the run passes anyway, which is worse than a real failure: it trains
// you to ignore errors in the log. DC defines SYNTHESIS automatically;
// simulators do not, so the tracer still builds for the testbench.
`ifndef SYNTHESIS
