...
// T22NM selects the memory implementation in rtl/soc/perips/{rom,ram}.sv:
//   defined    -> vendor SRAM macros (sram_sp_16384x32 / sram_sp_8192x32).
//                 This is the path the FI campaign was measured on, and the
//                 only practical one for synthesis.
//   undefined  -> the gen_ram behavioural model. No macro needed, so the
//                 design elaborates from a bare clone -- but 16k x 32 of
//                 inferred flops makes synthesis impractical, and none of
//                 the FI numbers in docs/ were measured on it.
// Deliberately NOT defined here: pass it on the command line so one switch
// reaches the RTL, the testbench and the synthesis flow together.
//   sim :  sim/config.mk  ->  DEFINES
//   syn :  syn/tool_config.mk -> RTL_DEFINES
//`define T22NM
