...
//
// Modified for FIawase: a `scan_gate` freeze input was added to the port
// list (the DUT side of the wrapper's Clk_Gate -- see FI_GATE_PORT in
// syn/fi_scan_cfg.tcl), and uart0 was swapped for scan_uart_top, whose
// FIFO + CDC keeps the UART transmitting while the rest of the SoC is
// frozen for a scan window. The clock gating itself is NOT here: it is
// spliced into the netlist by the synthesis flow. See docs/DESIGN.md 6.
...
module tinyriscv_soc_top(
    ...
    input wire                io_testmode,
    output wire               halted_ind_pin,          // jtag是否已经halt住CPU，高电平有效

    input   wire[16-1:0] io_data_in,                 // IO引脚，1bit代表一个IO
    output  wire[16-1:0] io_data_out,                 // IO引脚，1bit代表一个IO
    output  wire[16-1:0] io_oe,                 // IO引脚，1bit代表一个IO
...
`endif  
    ...
    // No clock gating here on purpose.
    //
    // Which blocks are frozen during the FI scan window is declared in
    // syn/fi_scan_cfg.tcl (FI_GATE_PINS) and spliced into the
    // netlist by fi_insert_clock_gate. Keeping it out of the RTL is
    // what makes the wrapper portable -- a DUT you did not write can be
    // gated without editing it -- and it removes the failure mode where
    // the RTL gates a block that synthesis still stitches into the scan
    // chain (which makes the chain unrestorable, silently).
    //
    // The scan_gate input port stays: it is the wrapper's freeze
    // request, and uart0 consumes it directly for its FIFO/CDC
    // protection. That port is the only thing the FI flow requires a
    // DUT to expose.

    localparam int          TRACE_ENABLE         = 1'b0;
    localparam int          GPIO_NUM             = 16;
    localparam int          I2C_NUM              = 2;
    localparam int          UART_NUM             = 3;
    localparam int          SPI_NUM              = 1;
...
`endif
    ...
    wire[GPIO_NUM-1:0] gpio_data_out_module;
...
`endif
    ...
    wire scan_clk;
    assign scan_clk = clk_50m_i;    // is it true?

    assign clk = clk_50m_i;                         // halt level control
    ...
    gpio_top #(
        ...
        .rst_ni         (ndmreset_n),
        .gpio_oe_o      (gpio_oe),
        .gpio_data_o    (gpio_data_out_module),
    ...
    // assign gpio_data_out = io_testmode ? {GPIO_NUM{1'b0}} : gpio_data_out_module;
    assign gpio_data_out =  gpio_data_out_module;
    ...
    scan_uart_top uart0 (
        ...
        .clk_tx_i    (clk),
        .rst_ni      (ndmreset_n),
        .scan_gate_i (scan_gate),   // raw scan_gate from fi_wrapper
        .rx_i        (uart_rx[0]),
        .tx_o        (uart_tx[0]),
        .irq_o       (uart0_irq),
...
`endif
    ...
    // assign ndmreset_n = io_testmode ? rst_ext_ni : rst_sc_ctrl;
    // assign ndmreset_n = rst_ext_ni;
    assign ndmreset_n = rst_sc_ctrl;
