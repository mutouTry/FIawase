...
module jtag_dm #(
    ...
    // DM regs
    reg[31:0] data0_d, data0_q;                                 // abstract data 0
    reg[31:0] dmcontrol_d, dmcontrol_q;                         // debug module control
    reg[31:0] dmstatus;                                         // debug module status
    reg[31:0] abstractcs;                                       // abstract control and status
    reg[31:0] command_d, command_q;                             // abstract command
    reg[31:0] abstractauto_d, abstractauto_q;

    reg[31:0] sbaddress0_d, sbaddress0_q;                       // system bus address 
    reg[31:0] sbcs_d, sbcs_q;                                   // system bus access control and status 
        ...
        if (dmi_valid_i & dm_ready_o) begin
            ...
            if (dm_op == `DMI_OP_READ) begin
                case (dm_op_addr)
                    ...
                    // `ScanWaitStart: dm_resp_data_d = {scanchain_start_q, scanchain_wait_q};
