...
module scan_uart_top (
    input  logic        clk_tx_i,
    input  logic        clk_i,
    input  logic        rst_ni,

    // Raw scan_gate from top-level fi_wrapper (unsynchronized).
    // Used internally to pause UART tx handshake across the scan window,
    // avoiding CDC boundary corruption when clk_i is gated.
    ...
    scan_uart_core u_uart_core (
        .clk_tx_i   (clk_tx_i),
        .clk_i      (clk_i),
        .rst_ni     (rst_ni),
        .scan_gate_i(scan_gate_i),                  // new
        .tx_pin_o   (tx_o),
        .rx_pin_i   (rx_i),
        .irq_o      (irq_o),
        .reg_we_i   (we),
