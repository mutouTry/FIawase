...
module rvic_core (
    ...
    rvic_reg2hw_t reg2hw;
    rvic_hw2reg_t hw2reg;
    ...
    int_info_t l4_max[2];
    always_comb begin
        for (int unsigned i = 0; i < 2; i = i + 1) begin
    ...
    // 最多32个中断源
    wire [4:0] id;
    assign id = irq_id[4:0];

    // 硬件置位pending
    assign hw2reg.pending.de = src_i[id] & irq_enable[id];
    assign hw2reg.pending.d  = irq_pending | (1'b1 << id);
