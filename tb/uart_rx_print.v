...
//
// Original uart_recv by WangFW, whose copyright in the unmodified parts
// stands. Changed for FIawase: the USE_FILE / LOGFILE parameters and the
// per-instance file logging they drive, so several taps can observe the
// same UART at once without interleaving into stdout. See NOTICE.
//-------------------------------------------------------------------
...
// NOTE (2026-07-27): this receiver does NOT check the stop bit -- it
// emits a byte unconditionally once rx_cnt reaches 9. start_flag is a
...
integer print_fd;
initial begin
	if (USE_FILE) print_fd = $fopen(LOGFILE, "w");
	else          print_fd = 32'h80000002;
end

always @(posedge sys_clk, negedge sys_rst_n) begin
	if((uart_done_d1== 1'b1) & (uart_done_d2 == 1'b0) )
		$fwrite(print_fd, "%c", uart_data);
end
